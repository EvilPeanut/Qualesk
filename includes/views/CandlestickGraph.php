		...
		<script>
			...
			*/
			...
			foreach ( $sensor_readings as $uuid => $sensor_reading ) {
				$dateTime = new DateTime( $sensor_reading[ 'date' ] );
				$date = $dateTime->format( 'Y-m-d' );
				$previousDateTime = new DateTime( $previousReading[ 'date' ] );
				$data = $sensor_reading[ 'data' ];

				if ( is_null( $currentHigh ) || $data > $currentHigh ) {
					$currentHigh = $data;
				}

				if ( is_null( $currentLow ) || $data < $currentLow ) {
					$currentLow = $data;
				}

				if ( is_null( $previousReading ) || $date != $previousDateTime->format( 'Y-m-d' ) ) {
					if ( !is_null( $previousReading ) ) {
						$currentDataObject[ 'low' ] = $currentLow;
						$currentDataObject[ 'high' ] = $currentHigh;
						$currentDataObject[ 'close' ] = $previousReading[ 'data' ];
						array_push( $chartData, $currentDataObject );
					}
					...
					$currentDataObject[ 'open' ] = $data;
				}

				$previousReading = $sensor_reading;
			}
			...
			foreach ( $chartData as $data_object ) {
				echo "{ date: new Date( Date.parse( '" . $data_object[ 'date' ] . "' ) ), open: " . $data_object[ 'open' ] . ", high: " . $data_object[ 'high' ] . ", low: " . $data_object[ 'low' ] . ", close: " . $data_object[ 'close' ] . "},";
			}
			...
			var graph_unit = "<? echo $sensor[ 'unit' ] ?>";
			...
			series.tooltipText = "Open: {openValueY.value} " + graph_unit + "\nLow: {lowValueY.value} " + graph_unit + "\nHigh: {highValueY.value} " + graph_unit + "\nClose: {valueY.value} " + graph_unit;
